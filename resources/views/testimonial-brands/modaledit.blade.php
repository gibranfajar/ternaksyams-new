...
<div class="modal fade" id="editTestimonialModal{{ $testimonial->id }}" tabindex="-1"
    ...
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            ...
            <form action="{{ route('testimonial-brands.update', $testimonial->id) }}" method="POST"
                ...
                <div class="modal-body">
                    ...
                    <div class="row mb-3">
                        <div class="col-md-6">
                            <label for="brand{{ $testimonial->id }}" class="form-label">Brand</label>
                            <select name="brand" id="brand{{ $testimonial->id }}" class="form-select">
                                <option value="">-- Select Brand --</option>
                                @foreach ($brands as $item)
                                    <option value="{{ $item->id }}"
                                        {{ $item->id == $testimonial->brand_id ? 'selected' : '' }}>
                                        {{ $item->brand }}</option>
                                @endforeach
                            </select>
                        </div>
